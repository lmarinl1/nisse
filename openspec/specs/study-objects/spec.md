...
Opening a Study SHALL enter its Workspace. A newly created Workspace SHALL present an empty Canvas prepared as structure for future capabilities. The Workspace MUST NOT yet implement Cognitive Objects, AI Companion content, scenarios, journals, visualizations, or methodologies.

#### Scenario: Open newly created Workspace
- **WHEN** the user opens a newly created Study
- **THEN** the Workspace shows an empty Canvas as the primary research surface without those deferred capabilities

#### Scenario: Canvas remains protagonist
- **WHEN** the user is inside a Study Workspace
- **THEN** the Canvas occupies the primary visual space consistent with the NISSE Workspace grammar
